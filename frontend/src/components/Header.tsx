...
import { getNotifications, type Notification } from "../app/(modules)/communication/calendar/api/notificationApi";

const Header: React.FC = () => {
  // -------------------------
  // Estados de interface
  // -------------------------
  const [query, setQuery] = useState(""); // barra de pesquisa
  ...
  useEffect(() => {
    function handleDocClick(e: MouseEvent) {
      ...
      if (msgRef.current && !msgRef.current.contains(target)) setShowMessages(false);
    }
  ...
  return (
    <header className="flex items-center justify-between h-16 px-6 bg-gradient-to-r from-gray-950 to-gray-900 border-b border-gray-800 shadow-[0_1px_10px_rgba(0,0,0,0.3)] relative z-10">
            ...
            {filteredResults.length > 0 ? (
              filteredResults.map((menu) => (
                <div key={menu.section} className="px-3 py-2">
                  ...
                  {menu.items.map((item) => (
                    <a key={item} href="#" className="block px-3 py-1 text-sm text-gray-300 hover:bg-blue-600/20 hover:text-blue-400 rounded-md transition">{item}</a>
                  ))}
                </div>
              ))
            ) : (
      ...
      {/* Ações: notificações, mensagens, perfil */}
      <div className="flex items-center gap-5 relative">
        ...
        <div ref={notifRef} className="relative">
          ...
          >
            ...
            {unreadCount > 0 && <span className="absolute -top-1 -right-1 bg-red-500 rounded-full w-2.5 h-2.5 animate-pulse"></span>}
          </button>

          {/* Dropdown de notificações */}
          {showNotifications && (
            <div onClick={(e) => e.stopPropagation()} className="absolute right-0 top-full mt-2 w-72 bg-gray-900/95 border border-gray-700/50 rounded-lg shadow-lg animate-fade-in backdrop-blur-md z-30">
              <div className="p-3 border-b border-gray-800 flex justify-between items-center">
                <h4 className="text-sm font-semibold text-gray-200">Notificações</h4>
                <a href="/notifications" className="text-xs text-blue-400 hover:text-blue-300">Ver todas</a>
              </div>
              ...
              {notifications.length === 0 ? (
                ...
                <ul className="max-h-64 overflow-y-auto">
                  {notifications.slice(0, 5).map((n) => (
                    <li key={n.id} className={`p-3 text-sm border-b border-gray-800 last:border-none ${n.read ? "text-gray-400" : "text-white bg-blue-900/30"}`}>{n.message}</li>
                  ))}
                </ul>
              )}
            </div>
        ...
        <div ref={msgRef} className="relative">
          ...
          {showMessages && (
            <div onClick={(e) => e.stopPropagation()} className="absolute right-0 top-full mt-2 w-72 bg-gray-900/95 border border-gray-700/50 rounded-lg shadow-lg animate-fade-in backdrop-blur-md z-30">
              <div className="p-3 border-b border-gray-800"><h4 className="text-sm font-semibold text-gray-200">Mensagens</h4></div>
        ...
        <div ref={profileRef} className="relative flex items-center">
          ...
          {showProfile && (
            <div onClick={(e) => e.stopPropagation()} className="absolute right-0 top-full mt-2 w-56 bg-gray-900/95 border border-gray-700/50 rounded-lg shadow-lg animate-fade-in backdrop-blur-md z-30">
              <div className="p-3 border-b border-gray-800"><h4 className="text-sm font-semibold text-gray-200">{fullName}</h4></div>

              {/* Editar perfil */}
              <button onClick={(e) => e.stopPropagation()} className="w-full text-left px-4 py-2 text-sm text-gray-300 hover:bg-blue-600/20 hover:text-blue-400 transition">✏️ Editar Perfil</button>
              ...
              <div className="border-t border-gray-800 mt-1 pt-1">
                ...
                {[
                  ...
                  <button key={s.key} onClick={(e) => { e.stopPropagation(); setStatus(s.key as "online" | "busy" | "offline"); setShowProfile(false); }} className={`flex items-center gap-2 px-4 py-2 w-full text-sm ${status === s.key ? "text-blue-400" : "text-gray-300"} hover:bg-blue-600/20 hover:text-blue-400 transition`}>
              ...
              <div className="border-t border-gray-800 mt-1">
                <button onClick={logout} className="w-full text-left px-4 py-2 text-sm text-red-400 hover:bg-red-600/20 hover:text-red-500 transition">⏻ Sair</button>
              </div>
            </div>
          )}
        </div>
